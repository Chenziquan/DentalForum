package jqchen.dentalforum.http.cos;
...
import jqchen.dentalforum.base.UploadPictureCallBack;
import jqchen.dentalforum.http.cos.exception.AbstractCosException;

/**
 * Created by jqchen on 2016/10/13.
 * Use to
 */
public class PutObjectSamples extends AsyncTask<BizServer, Long, String> {
    ...
    public PutObjectSamples(UploadPictureCallBack callBack, PUT_TYPE put_type) {
...
//        progressText.setText("下载进度 :" + values[0].intValue() + "%");
     ...
     * 一键上传
     */
    protected void putObjcetForListFile(BizServer bizServer) throws AbstractCosException {
        ...
        for (int i = 0; i < listPath.size(); i++) {
            ...
            putObjectRequest.setSign(Sign.getSign(bizServer.getFileId() + File.separator + FileUtils.getFileName(listPath.get(i))));
            putObjectRequest.setListener(iUploadListenerHandler);
    ...
    protected class IUploadListenerHandler implements IUploadTaskListener {
        ...
        public void onSuccess(COSRequest cosRequest, COSResult cosResult) {
            ...
            resultStr = (" 上传结果： ret=" + result.code + "; msg =" + result.msg + "\n") +
                    " access_url= " + (result.access_url == null ? "null" : result.access_url) + "\n" +
                    " resource_path= " + (result.resource_path == null ? "null" : result.resource_path) + "\n" +
                    " url= " + (result.url == null ? "null" : result.url);
            callBack.onSuccess(result.access_url);
        }

        @Override
        public void onFailed(COSRequest cosRequest, COSResult cosResult) {
            resultStr = "上传出错： ret =" + cosResult.code + "; msg =" + cosResult.msg;
            callBack.onFail(resultStr);
        }
    }
}
